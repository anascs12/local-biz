...
import { deterministicAnswer } from "@/lib/ai/deterministic";

/** Marks which replies were computed rather than generated by a model. */
export type MessageSource = "ai" | "computed";

export interface UseAIChatResult {
  messages: ChatMessage[];
  /** Parallel to `messages`: how each assistant reply was produced. */
  sources: Record<number, MessageSource>;
  aiAvailable: boolean;
...
export function useAIChat(): UseAIChatResult {
  ...
  const [sources, setSources] = React.useState<Record<number, MessageSource>>({});
  ...
  const run = React.useCallback(
    async (history: ChatMessage[]) => {
      ...
      // No key on this deployment → answer from the computed analytics rather
      // than showing an error. Grounded in the same numbers, labelled as computed.
      const lastQuestion = [...history].reverse().find((m) => m.role === "user")?.content ?? "";
      if (aiStatus === "disabled" && dataset && analytics) {
        const answer = deterministicAnswer(lastQuestion, dataset, analytics);
        setMessages((prev) => {
          setSources((s) => ({ ...s, [prev.length]: "computed" }));
      ...
      try {
        ...
        if (!response.ok || !response.body) {
          // Prefer a grounded computed answer over a dead end.
    ...
    [context, aiStatus, dataset, analytics],
  );
